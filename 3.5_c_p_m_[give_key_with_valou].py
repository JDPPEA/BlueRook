...
while follow:
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONDOWN:
            cordonnate.append(event.pos[0])
            cordonnate.append(event.pos[1])
            true_cordonnate = center_square()
            cordonnate=[]
        if event.type == MOUSEBUTTONUP:
            if event.button == 1:
                print(give_key_with_valou(true_cordonnate))
            
        
    refresh()           
    

#Cette version 3.5 amène la fct give_key_with_valou elle donne la clé en connaissant la valeur dans un dico
